...
from market_scout.base_models import ConsensusOpportunity, TradingOpportunity, ValidationResult
...
class TestDisplayOpportunities:
    ...
    def test_displays_no_opportunities_message(self, capsys):
        ...
        result = ValidationResult(opportunities=[], consensus_opportunities=[], model_count=1)
    ...
    def test_displays_multiple_opportunities_grouped_by_symbol(self, capsys):
        ...
        result = ValidationResult(
            opportunities=[opp1, opp2, opp3], consensus_opportunities=[], model_count=2
        )
...
class TestDisplayConsensusOpportunities:
    ...
    def test_displays_consensus_opportunity(self, capsys):
        ...
        result = ValidationResult(
            opportunities=[], consensus_opportunities=[consensus], model_count=3
        )
...
class TestMain:
    ...
    def test_main_with_symbol_not_found_error(
        ...
        mock_analyzer_instance = Mock()
        mock_analyzer.return_value = mock_analyzer_instance
        mock_analyzer_instance.analyze_symbol.side_effect = SymbolNotFoundError(
    ...
    def test_main_with_service_unavailable_error(
        ...
        mock_analyzer_instance = Mock()
        mock_analyzer.return_value = mock_analyzer_instance
        mock_analyzer_instance.analyze_symbol.side_effect = ServiceUnavailableError(
    ...
    @patch("market_scout.cli.ModelRegistry")
    @patch("market_scout.cli.ValidationEngine")
    @patch("market_scout.cli.BollingerModel")
    @patch("market_scout.cli.MACDModel")
    @patch("market_scout.cli.RSIModel")
    @patch("market_scout.cli.NaiveModel")
    @patch("market_scout.cli.setup_logging")
    def test_main_initializes_all_components(
        ...
        mock_rsi_model_class,
        mock_macd_model_class,
        mock_bollinger_model_class,
        ...
        mock_rsi_model = Mock()
        mock_macd_model = Mock()
        mock_bollinger_model = Mock()
        ...
        mock_rsi_model_class.return_value = mock_rsi_model
        mock_macd_model_class.return_value = mock_macd_model
        mock_bollinger_model_class.return_value = mock_bollinger_model

        # Mock get_all_models to return a list
        mock_registry.get_all_models.return_value = [
            mock_naive_model,
            mock_rsi_model,
            mock_macd_model,
            mock_bollinger_model,
        ]

        mock_analyzer.analyze_symbol.return_value = ValidationResult(
            opportunities=[], consensus_opportunities=[], model_count=4
        )
        ...
        mock_rsi_model_class.assert_called_once()
        mock_macd_model_class.assert_called_once()
        mock_bollinger_model_class.assert_called_once()

        assert mock_registry.register.call_count == 4
        mock_registry.register.assert_any_call(mock_naive_model)
        mock_registry.register.assert_any_call(mock_rsi_model)
        mock_registry.register.assert_any_call(mock_macd_model)
        mock_registry.register.assert_any_call(mock_bollinger_model)
    ...
    def test_main_displays_consensus_opportunities(
        ...
        result = ValidationResult(
            opportunities=[], consensus_opportunities=[consensus], model_count=2
        )
