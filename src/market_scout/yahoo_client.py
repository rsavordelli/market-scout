...
from .base_models import HistoricalData
from .exceptions import NetworkError, ServiceUnavailableError, SymbolNotFoundError
    ...
    def fetch_historical_data(self, symbol: str, period: str = "1mo") -> HistoricalData:
        ...
        try:
            ...
            if not required_cols.issubset(actual_cols):
                ...
                raise ValueError(f"Data for '{symbol}' is missing required columns: {missing}")
            ...
            logger.info(
                f"Successfully fetched {len(df)} data points for {symbol} (period: {period})"
            )
            ...
            raise NetworkError(
                f"Network connectivity failed while fetching data for '{symbol}'", original_error=e
            ) from e
            ...
            raise NetworkError(
                f"Request timed out while fetching data for '{symbol}'", original_error=e
            ) from e
            ...
            if "404" in error_str or "not found" in error_str:
                ...
                raise SymbolNotFoundError(
                    symbol, f"Symbol '{symbol}' not found in Yahoo Finance"
                ) from e
            ...
            if any(code in error_str for code in ["500", "502", "503", "504"]):
                ...
                raise ServiceUnavailableError(
                    "Yahoo Finance API is currently unavailable", status_code=None
                ) from e
            ...
            raise ServiceUnavailableError(f"Failed to fetch data for '{symbol}': {str(e)}") from e
