...
from .base_models import ConsensusOpportunity, TradingOpportunity, ValidationResult
...
class ValidationEngine:
    ...
    def validate(self, opportunities: list[TradingOpportunity]) -> ValidationResult:
        ...
        if not opportunities:
            ...
            return ValidationResult(opportunities=[], consensus_opportunities=[], model_count=0)
        ...
        for opp in opportunities:
            try:
                ...
                invalid_count += 1
                logger.warning(f"Filtered out opportunity with missing/invalid fields: {e}")
                continue
        ...
        # Count unique models
        unique_models = {opp.model_id for opp in valid_opportunities}
        ...
        for symbol, symbol_opps in opportunities_by_symbol.items():
            if len(symbol_opps) >= 2:
                ...
                for group in consensus_groups:
                    if len(group) >= 2:
                        consensus = self._create_consensus_opportunity(symbol, group, model_count)
    ...
    def _create_consensus_opportunity(
        self, symbol: str, opportunities: list[TradingOpportunity], total_models: int
