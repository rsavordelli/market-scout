...
def display_opportunities(result) -> None:
    ...
    for symbol, opps in opps_by_symbol.items():
        ...
        for opp in opps:
            ...
            print(
                f"  Data Period: {data_days} days ({opp.data_period_start.strftime('%Y-%m-%d')} to {opp.data_period_end.strftime('%Y-%m-%d')})"
            )
            print()
            print("  Reasoning:")
            ...
            print(
                f"    Stop Loss:         ${opp.stop_loss_price:>12.2f}  ({stop_loss_pct:>5.1f}% risk)"
            )
            print(
                f"    Gain Target:       ${opp.gain_target_price:>12.2f}  ({gain_target_pct:>5.1f}% gain)"
            )
            print()
...
def main() -> int:
    ...
    try:
        ...
        # Register all models with registry
        naive_model = NaiveModel()
        rsi_model = RSIModel()
        macd_model = MACDModel()
        bollinger_model = BollingerModel()

        registry.register(naive_model)
        registry.register(rsi_model)
        registry.register(macd_model)
        registry.register(bollinger_model)

        logger.info(f"Registered {len(registry.get_all_models())} model(s)")
