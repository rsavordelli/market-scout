...
from market_scout.base_models import PredictionModel
...
class ModelRegistry:
    ...
    def register(self, model: PredictionModel) -> None:
        ...
        if any(m.model_id == model.model_id for m in self._models):
            logger.error(f"Attempted to register duplicate model ID: {model.model_id}")
            raise ValueError(f"Model with ID '{model.model_id}' is already registered")

        self._models.append(model)
        logger.info(f"Registered model: {model.model_id} (total models: {len(self._models)})")
