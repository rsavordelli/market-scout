...
from .exceptions import SymbolNotFoundError, ServiceUnavailableError
from .model_registry import ModelRegistry
from .models import ValidationResult, TradingOpportunity
...
class Analyzer:
    ...
    def analyze_symbol(self, symbol: str) -> ValidationResult:
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(historical_data.data)} data points for {symbol}"
            )
        ...
        if failed_models:
            logger.warning(
                f"The following models failed for {symbol}: {', '.join(failed_models)}"
            )
